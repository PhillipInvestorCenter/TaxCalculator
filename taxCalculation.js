...
function calculateTax() {
  ...
  // insurance + investment (always read; zeros ignored)
  ...
  // stimulus (always read)
  let total_stimulus_deductions = 0;
  let easyVal = 0, travelMainSecondaryVal = 0, homeLoanVal = 0, newHomeVal = 0, newHomeDeduction = 0, solarVal = 0;
  ...
  total_stimulus_deductions = easyVal + travelMainSecondaryVal + homeLoanVal + newHomeDeduction + solarVal;
  ...
  // cap donation at 10% of taxable income
  ...
  const ctx = {
    ...
    easyVal, travelMainSecondaryVal, homeLoanVal, newHomeDeduction, solarVal,
  };
...
function renderTaxWaterfall(totalIncome, netIncome, expenseVal, personalAllowance){
  ...
  // Nice step helper → rounds UP so (step*10) ≥ target
  function niceStepFor(target, desired = 10){
    if (target <= 0) return 10000;
    const raw = target / desired;                         // desired ≈ 10 ticks
    const pow10 = Math.pow(10, Math.floor(Math.log10(raw)));
    const frac = raw / pow10;
    // include 1.2/1.25/1.5 to get 120k/125k/150k style steps
    const choices = [1, 1.2, 1.25, 1.5, 2, 2.5, 5, 10];
    let pick = choices[choices.length - 1];
    for (let i = 0; i < choices.length; i++){
      if (frac <= choices[i]) { pick = choices[i]; break; }
    }
    return pick * pow10;
  }
